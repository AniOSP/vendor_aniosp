...
def get_from_manifest(device_name):
    if os.path.exists(aniosp_local_manifest):
        man = load_manifest(aniosp_local_manifest)
...
def add_to_manifest(repos, fallback_branch=None):
    lm = load_manifest(aniosp_local_manifest)
    ...
    for repo in repos:
        ...
        if 'branch' in repo:
            ...
            repo_branch=aniosp_default_revision
        if 'remote' in repo:
    ...
    f = open(aniosp_local_manifest, 'w')
    f.write(raw_xml)
    f.close()
...
def fetch_dependencies(repo_path, fallback_branch=None):
    ...
    dep_p = '/'.join((repo_path, aniosp_dependencies))
    ...
    for dependency in dependencies:
        if not is_in_manifest(dependency['target_path']):
            if not dependency.get('branch'):
                dependency['branch'] = aniosp_default_revision
...
def detect_revision(repo):
    ...
    print("Calculated revision: %s" % aniosp_default_revision)

    if has_branch(result, aniosp_default_revision):
        return aniosp_default_revision

    print("Branch %s not found" % aniosp_default_revision)
    sys.exit()
